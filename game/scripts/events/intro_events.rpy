...
label first_potion_courtyard_event (**kwargs):
    ...
    headmaster.think "I certainly enjoy the view. Unfortunately it only lasts for today until the serum finishes settling in their bodies."
...
label first_week_courtyard_event (**kwargs):
    ...
    headmaster.think "Hmm, the courtyard looks really bad..."
    
    $ image.show(2)
    headmaster.think "It seems most of the appliances here are out of order."

    $ image.show(3)
    headmaster.think "For example the public toilet is broken."

    $ image.show(4)
    headmaster.think "At least the courtyard doesn't need immediate fixing."
...
label first_potion_gym_event (**kwargs):
    ...
    $ image.show(2)
    headmaster.think "Oh that is a sport session I can get behind!"

    $ image.show(3)
    headmaster.think "Mhh, yes very flexible!"

    $ image.show(4)
    headmaster.think "Oh they seem to really get into it!"
...
label first_week_gym_event (**kwargs):
    ...
    $ image.show(1)
    headmaster.think "Okay, now the Gym. I have been here shortly for my introduction speech but I haven't had the chance to get a thorough look."

    $ image.show(2)
    headmaster.think "Mhh, doesn't look to shabby..."
    
    $ image.show(3)
    headmaster.think "Seems to be decently stocked."
    headmaster.think "The material is well maintained. I guess it's alright."
...
label first_week_kiosk_event (**kwargs):
    ...
    $ image.show(1)
    headmaster.think "Now, somewhere here should be the kiosk..."
    $ image.show(2)
    headmaster.think "Hmm, why is it so crowded?"
    ...
    $ image.show(5)
    headmaster.think "This is not acceptable. Did the former headmaster really close the kiosk?"
    headmaster.think "That can't be right..."
...
label first_potion_office_building_event (**kwargs):
    ...
    headmaster.think "Ahh the teacher seem to be eating at the kiosk as well."
    $ image.show(2)
    headmaster.think "Not that I have a problem with it. Quite the opposite. That makes some things a bit easier."
...
label first_week_sb_event (**kwargs):
    ...
    $ image.show(2)
    headmaster.think """Despite my fear, the building seems to be rather well maintained.
    ...
    $ image.show(3)
    headmaster.think "Oh not bad as well. "

    $ image.show(4)
    headmaster.think "Hmm I think there should be a class right now, let's check."

    $ image.show(6)
    headmaster.think "Hmm looks like a normal class, but I think the students have no material?"
    headmaster.think "Yeah, not one school girl has even one book."
    headmaster.think "I guess the former headmaster cut back on those"
...
label first_potion_sb_event (**kwargs):
    ...
    $ image.show(1)
    headmaster.think "Let's see how classes are today."
    ...
    headmaster.think "Hmm, I can't wait to have this view on a regular basis, but that's gonna take some time."
...
label first_week_school_dormitory_event (**kwargs):
    ...
    $ image.show(1)
    headmaster.think "The dormitory looks alright."

    $ image.show(2)
    headmaster.think "As far as I know, the students have to share a communal bathroom."
    headmaster.think "Private bathrooms would be nice for the students, but for one I don't think we really need that and then it would need a lot of rebuilding. So that should be last on the list."
    
    $ image.show(3)
    headmaster.think "Let's see if someone would let me see their room so I can check the state of these."
    ...
    $ image.show(5)
    headmaster.think "Hmm nobody seems to be here. Nevermind. I just let my Secretary give me a report."
...
label first_potion_school_dormitory_event (**kwargs):
    ...
    headmaster.think "Mhh, where does the noise come from?"

    $ image.show(2)
    headmaster.think "Ah I think there are some students in the room over there."

    $ image.show(3)
    headmaster.think "Ahh party games!"

    $ image.show(4)
    if time.check_daytime("c"):
        headmaster.think "Normally I would scold them for skipping class but today is a special day so I gladly enjoy this view."
    else:
        headmaster.think "Ahh I like this view. Nothing more erotic than nudity in combination with a party game."
...
label .start:
    ...
    $ image.show(3)
    emiko """Hello Mr. [headmaster_last_name], nice to meet you!
        My name is [secretary_name].
    """ (name="Secretary")
    emiko "From now on I'll be your secretary."

    $ image.show(0)
    emiko """I used to work for the previous headmaster, 
        so I know the school pretty well.
    """

    $ image.show(4)
    emiko "If you have any questions just come and ask me."
    ...
    $ image.show(4)
    emiko "So now it is your job to go on and fix this school!"
    ...
    $ image.show(1)
    emiko """But new rules must be approved by the PTA which is made up of the school council, teachers and a 
    ...
    $ image.show(2)
    emiko "Now you know the basics. You might want to hurry down to the gym for the weekly meeting."

    $ image.show(4)
    emiko "I'm sure the students are eager to meet you."
    ...
    emiko.whisper "The students in the left are from the high school."
    # if loli_content >= 1:
    ...
    headmaster.think "Wow she has a nice butt. I can't wait to make it mine."
    ...
    emiko.shout "Good Morning Students!"
    ...
    emiko.shout "It is with great pleasure, that I introduce you to your new Headmaster."
    emiko.shout "He will be starting today and we're all very excited to see the positive changes he will bring!"

    #show view from slightly behind crowd towards stage (secretary pointing towards headmaster)
    emiko.shout "But without further ado... Greet your new Headmaster Mr. [headmaster_last_name]!"
    ...
    headmaster.shout "Greetings to you all. I am honored to stand here today."
    headmaster.shout "When I came here, I saw the condition of this school and the mishaps of your former headmaster."
    headmaster.shout "I guarantee I won't repeat the same mistakes and I will bring this school back to its former glory and beyond."
    ...
    headmaster.shout "Now a little about me. My name is [headmaster_first_name] [headmaster_last_name] and I come from the country."
    headmaster.shout "I taught in various schools and was also the headmaster of a school in the capitol."

    #show headmaster rasing finger
    headmaster.shout "I want to make it clear that I will not tolerate misbehavior! {b}BUT{/b} I am a fair person."
    headmaster.shout "No one will be punished if they don't deserve it, and I will do everything I can to make sure that you all have a nice and safe place to grow and learn!"

    #show headmaster with wide open arms
    headmaster.shout "If you ever have any problems, ideas or questions, feel free to come to me anytime. I will help wherever I can!"
    headmaster.shout "Thank you and let's have a great time together!"
    crowd_shout "*clapping*"

    $ image.show(4)
    emiko "Wow! That was a nice speech!"
    emiko "Now that we finished the introduction, let's start with the entry paperwork."
    headmaster "Alright."
